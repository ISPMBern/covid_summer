...
model_outputs_01 <- su2020_cases(Re_all, 0.1)
model_outputs_01 <- as.data.frame(do.call(rbind, lapply(model_outputs_01, `length<-`, max(lengths(model_outputs_01)))))

model_outputs_1 <- su2020_cases(Re_all, 1)
model_outputs_1 <- as.data.frame(do.call(rbind, lapply(model_outputs_1, `length<-`, max(lengths(model_outputs_1)))))

if(i=="2020"){
  saveRDS(model_outputs_01, paste0("sensitivity01_noimport_model_output2020_",Sys.Date(),".rds"))
  saveRDS(model_outputs_1, paste0("sensitivity1_noimport_model_output2020_",Sys.Date(),".rds"))
}
else if(i=="2021"){
  saveRDS(model_outputs_01, paste0("sensitivity01_noimport_model_output2021_",Sys.Date(),".rds"))
saveRDS(model_outputs_1, paste0("sensitivity1_noimport_model_output2021_",Sys.Date(),".rds"))
}
}
