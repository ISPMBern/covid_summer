...
su2020_imports <- function(Re, dispersion, imports){
    ...
    foreach(Ri=1:length(Re)) %dopar% {#lapply(Re,function(R) {
      R <- Re[Ri]
      k <- sample(dispersion_parameters, size=1)#for (i in 1:length(dispersion[1:2])) { #sapply(1:length(dispersion),function(i) {
        secondary_t <- ti
        ...
        model_outputs[Ri,]<- c(noimport_model_output[grep(R,noimport_model_output[,3]),1],import_num[z+2],R,noimport_model_output[grep(R,noimport_model_output[,3]),4],k,(sum(cases_d_runs[,1])+noimport_model_output[grep(R,noimport_model_output[,3]),6]),cases_d_runs[max_time,1]+noimport_model_output[grep(R,noimport_model_output[,3]),7], (cases_d_runs[,1]+noimport_model_output[grep(R,noimport_model_output[,3]),c(8:(max_time+7))]))
        return(model_outputs[Ri,])}
}
model_outputs <- su2020_imports(Re_all, dispersion_parameters, imports_d[,z+2])
model_outputs <- as.data.frame(do.call(rbind, lapply(model_outputs, `length<-`, max(lengths(model_outputs)))))
saveRDS(model_outputs, paste0("imports_model_outputs_",z,"_",Sys.Date(),".rds"))
